...
async def proxy_request(request: Request):
    ...
    async def generate_stream():
        ...
        async for chunk in stream_vllm_response(VLLM_2_URL, req_data):
            if chunk_count == 0:
                print(len(req_data['prompt']), req_data['prompt'][0:10], "----First chunk sent at:", datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
            chunk_count += 1
            yield chunk

        print(len(req_data['prompt']), req_data['prompt'][0:10], "----Last chunk sent at:", datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d %H:%M:%S.%f'))
        print(len(req_data['prompt']), req_data['prompt'][0:10], f"Total chunks sent: {chunk_count}")
